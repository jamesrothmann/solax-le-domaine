...
# ------------------------------------------------------------
# Config - secure values come from st.secrets
# ------------------------------------------------------------
...
XML_OUT = DATA_DIR / "solax_le_domaine_mii.xml"
...
# Scheduled times
HOUR_WINDOW_START = 4   # 04:00 SAST inclusive
HOUR_WINDOW_END = 20    # 20:00 SAST inclusive
...
# ------------------------------------------------------------
# Core: fetch markdown via Jina and parse "Daily yield"
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# CSV logging and interval kWh calculation
# ------------------------------------------------------------
...
def compute_interval(prev_daily_kwh, current_daily_kwh: float) -> float:
...
# ------------------------------------------------------------
# MII XML build and post
# ------------------------------------------------------------
def build_mii_xml(daily_kwh: float, when_utc: datetime) -> bytes:
    ...
    ET.SubElement(cfg, "next-scheduled-transfer").text = when_utc.replace(microsecond=0).isoformat().replace("+00:00","Z")
    ...
    dp = ET.SubElement(dps, "datapoint", attrib={
        "interval": "900",
        "timestamp": when_utc.replace(microsecond=0).isoformat().replace("+00:00","Z")
    })
    dv = ET.SubElement(dp, "device", attrib={"id": "inverter-1"})
    ET.SubElement(dv, "mv", attrib={"t": "E_DAY", "v": f"{daily_kwh:.3f}"})
...
def manual_xml_and_transfer() -> tuple[int, str]:
    last_row = read_last_row(CSV_FILE)
    if not last_row:
        raise RuntimeError("No CSV rows yet. Fetch at least once before generating XML.")
    kwh = float(last_row["daily_kwh"])
    xml_bytes = build_mii_xml(kwh, now_utc())
        ...
        try:
            ...
            if last_fetch_iso:
                try:
                    ...
                    already_fetched_this_hour = (last_fetch_local.year == local_now.year and
...
# ------------------------------------------------------------
# Cache the background thread so it survives reruns
# ------------------------------------------------------------
@st.cache_resource
def start_scheduler():
...
def apply_hacker_theme():
    ...
    <style>
      ...
      h1,h2,h3 { color: var(--accent) !important; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace; }
...
def render_dashboard(status_q: queue.Queue):
    st.title("⚡ Solax Daily Yield - Hacker Panel")
    ...
    with col3:
        st.markdown(f"<div class='metric'><b>Window</b><br>{HOUR_WINDOW_START:02d}:00 - {HOUR_WINDOW_END:02d}:00 SAST</div>", unsafe_allow_html=True)
    ...
    if CSV_FILE.exists():
        ...
        if not df.empty:
            st.caption(f"Rows: {len(df)} • Last log at {df.iloc[0]['timestamp_sast'] if 'timestamp_sast' in df.columns else df.iloc[0]['timestamp_utc']}")
    else:
        st.info("No CSV yet. Scheduler will log at the next scheduled time.")
